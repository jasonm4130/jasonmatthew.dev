...
Without hesitation. Workers, D1, Durable Objects, Queues, and Workflows compose into a coherent system that would take five AWS services and a PhD in IAM to replicate. The developer experience is good. The pricing is sane. And the fact that everything runs on one platform means there's one dashboard, one set of logs, and one billing relationship.
