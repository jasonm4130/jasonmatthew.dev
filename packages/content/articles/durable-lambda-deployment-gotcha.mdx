...
Here's the nuance. The alias-follows-latest behaviour isn't inherently broken. If you have a long-running pipeline and you notice something is wrong, or you improve the logic, you might _want_ the next checkpoint to pick up the new code. Hot-patching a running execution could be genuinely useful.
...
Durable Lambda is a genuine paradigm shift. Writing long-running stateful workflows in a single Lambda handler — without Step Functions, without managing state machines — is a meaningful simplification. We're running production AI inference pipelines on it and the developer experience is excellent.
