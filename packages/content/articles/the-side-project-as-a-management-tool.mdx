...
The value isn't theoretical. This weekend I built [FormRecap](https://formrecap.com) on Cloudflare Workers, managed the infrastructure with Terraform, and set up a [zero-secrets workflow](/writing/zero-secrets-in-git) using 1Password CLI's `op://` references so no credentials ever touch disk.
