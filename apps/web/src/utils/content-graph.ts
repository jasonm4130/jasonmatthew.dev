 ...
 * for external / unrecognised hrefs (left unvalidated). Articles route at `/writing/`
 * (the `blog` collection, renamed from `/blog/` in Stage C); projects route at
 * `/projects/`. The gate validates against the ACTIVE route set on purpose: a
 * `/writing/<missing>` link fails closed, and a legacy `/blog/<id>` href is NOT
 * quietly accepted here (it no longer routes on-site — the edge 301 forwards real
 * visitors, but a stale in-content link should surface as a build error). */
export function parseInternalHref(href: string): { collection: 'blog' | 'projects'; id: string } | null {
  // Strip a query/fragment first: `/writing/foo#section` and `/projects/foo?ref=x`
  ...
  const m = path.match(/^\/(writing|projects)\/([^/]+)\/?$/);
  if (!m) return null;
